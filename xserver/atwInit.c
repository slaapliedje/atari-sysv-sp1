/*
 * atwInit.c - screen side of the ATW800/2 ddx: map the card, set the
...
static long
atwEventTime(pScreen)
    ScreenPtr	pScreen;
{
    return atwLastEventTime;
}

extern void miPointerQueueEvent();
static miPointerCursorFuncRec atwPointerCursorFuncs = {
    atwEventTime,
    atwCursorOffScreen,
    atwCrossScreen,
    miPointerQueueEvent,
...
/*
 * The os layer selects on the ikbd fd for us; when it is readable, make
 * the two input-check counters differ so Dispatch calls
 * ProcessInputEvents. The block handler is the mandatory no-op.
 */
int atwInputPending, atwInputProcessed;
    ...
    pointer	pReadmask;
{
    ...
    if (atwIkbdFd >= 0 && (mask[atwIkbdFd / 32] & (1L << (atwIkbdFd % 32))))
	atwInputPending++;
}
    ...
    char	**argv;
{
    ...
    if (!miDCInitialize(pScreen, &atwPointerCursorFuncs))
	return FALSE;
    ...
    char	**argv;
{
    ...
    SetInputCheck(&atwInputProcessed, &atwInputPending);
}
